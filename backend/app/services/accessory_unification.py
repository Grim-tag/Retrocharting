...
def fix_schema():
    ...
    with engine.connect() as conn:
        try:
            # Add game_slug
            try:
                conn.execute(text("ALTER TABLE products ADD COLUMN IF NOT EXISTS game_slug VARCHAR"))
                print("   -> Checked column: game_slug")
            except Exception:
                # Fallback for older DBs without IF NOT EXISTS or invalid syntax
                try: 
            ...
            # Add variant_type
            try:
                conn.execute(text("ALTER TABLE products ADD COLUMN IF NOT EXISTS variant_type VARCHAR DEFAULT 'Standard'"))
                print("   -> Checked column: variant_type")
            except Exception:
                try: 
