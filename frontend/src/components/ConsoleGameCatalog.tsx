...
export default function ConsoleGameCatalog({
    ...
    // If currentPage is provided, we use Server Side Pagination mode
    // If not, we fall back to client-side (legacy behavior, but we want to phase this out for static site)
    ...
    // OPTIONAL: Only run this if we are NOT on a specific page? 
    // Actually, for Static Site, we WANT to disable this client-side fetching entirely if we are using pagination pages.
    // But we might want it for "Page 1" if we want to ensure freshness?
    // User issue is that this FAILS on static site. So we should DISABLE it for the Static Site build.
    // Simple check: if we have totalPages prop passed, we assume we are in "Static Pagination Mode".

    /* 
       DISABLED FOR STATIC SITE STABILITY
       The client-side fetching was breaking the static export (API not found).
       Now we rely on build-time generation + Pagination.
    */
    /*
    useEffect(() => {
       // ... (Original Logic Removed/Commented)
    }, ...);
    */
    ...
    const updateUrl = (search: string, genre: string, sort: string) => {
        ...
        // Ideally yes, but dealing with static paths is tricky.
        // For static site, we might just query string updates on the CURRENT page, 
        // relying on Client-Side filtering of the CURRENT page slice (sub-optimal but working)
        // OR we'd need to redirect to /games/[console] (Page 1).

        // Let's redirect to base console page if filtering (reseting page)
        // router.push(`/${lang}/${gamesSlug}/${systemSlug}?${params.toString()}`);
    ...
    // Note: This filters the DISPLAYED products (the current page slice). 
    // This is expected behavior for static pagination unless we have the full list.
    ...
    // Slice for display
    // If paginated server-side, we display ALL of filteredProducts (which is the page slice)
        ...
        <div>
            ...
            {currentPage && totalPages && totalPages > 1 && (
                <Pagination
                    ...
                    basePath={`/${lang}/${gamesSlug}/${systemSlug}`}
                />
            )}
            {/* FAQ Section */}
