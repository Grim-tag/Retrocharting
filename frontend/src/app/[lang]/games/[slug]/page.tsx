...
import ConsoleGameCatalog from '@/components/ConsoleGameCatalog'; // Re-enabled for SSR
import GameDetailView from '@/components/GameDetailView';
// import ConsoleGameCatalogWrapper from '@/components/ConsoleGameCatalogWrapper'; // Removed to allow SSR
    ...
    try {
        ...
        if (systemName) {
            ...
            try {
                ...
                // [FIX] Use Unified Rules (getGamesByConsole) instead of Legacy (getProductsByConsole)
                // This ensures we get 'slug' (game_slug) instead of just IDs, preventing Broken Links (404) on Static Site.
                const { getGamesByConsole, getProductsCountByConsole } = await import('@/lib/api');
                const [fetchedGames, fetchedGenres, count] = await Promise.all([
                    // Fetch top 50 games (SSR) to populate initial view and SEO
                    getGamesByConsole(systemName, ITEMS_PER_PAGE, undefined, 'title_asc', 0, undefined, 'game'),
                    getGenres(systemName),
                    getProductsCountByConsole(systemName, 'game')
                ]);

                // Adapt Games to Products for Catalog
                if (fetchedGames && fetchedGames.length > 0) {
                    products = fetchedGames.map((g: any) => ({
                        id: g.id,
                        pricecharting_id: 0,
                        product_name: g.title, // Unified uses 'title'
                        ...
                        genre: g.genre || "",
                        game_slug: g.slug // CRITICAL: This enables clean URLs
                    }));
                }
