...
// --- TYPES ---
type ScanStatus = 'loading' | 'found' | 'unknown' | 'contributed' | 'linked';
...
// --- MAIN PAGE COMPONENT ---
export default function BatchScanClient({ lang }: { lang: string }) {
    ...
    // Modal States
    const [selectedItemForContribute, setSelectedItemForContribute] = useState<ScannedItem | null>(null);
    const [selectedItemForLink, setSelectedItemForLink] = useState<ScannedItem | null>(null);
    ...
    const handleScan = (ean: string) => {
        setItems(prev => {
            if (prev.length > 0 && prev[0].ean === ean && (Date.now() - prev[0].timestamp < 2000)) return prev;
            if (prev.find(i => i.ean === ean)) return prev;
            playBeep();
    ...
    const handleContribute = async (formData: FormData) => {
        try {
            ...
            setSelectedItemForContribute(null);
        } catch (e: any) {
            console.error("Contribution Error", e);
            if (e.response?.status === 401) {
                alert("Vous devez être connecté pour contribuer. Retournez au menu pour vous connecter.");
            } else {
    ...
    const handleLinkProduct = async (product: any) => {
        ...
        try {
            // POST /products/{id}/ean?ean={code}
            await apiClient.post(`/products/${product.id}/ean?ean=${selectedItemForLink.ean}`);

            setItems(prev => prev.map(item =>
                item.ean === selectedItemForLink.ean ? { ...item, status: 'linked', product } : item
            ));

            alert(`Code-barres associé à ${product.product_name} !`);
            setSelectedItemForLink(null);
        } catch (e: any) {
            console.error("Link Error", e);
            if (e.response?.status === 401) {
                alert("Vous devez être connecté pour contribuer.");
            } else {
                alert("Erreur lors de l'association: " + (e.response?.data?.detail || e.message));
            }
        }
    };
    ...
    return (
        <div className="flex flex-col h-screen bg-black text-white overflow-hidden">
            ...
            <div className="relative h-[40%] bg-gray-900 border-b border-gray-800 overflow-hidden">
                    ...
                    <div className="absolute inset-0 z-50 bg-gray-900 flex items-center justify-center p-6 text-center">
                        <div className="max-w-xs w-full">
                            ...
                            <label className="block w-full bg-blue-600 hover:bg-blue-500 text-white font-bold py-4 rounded-lg shadow-lg cursor-pointer mb-3 flex items-center justify-center gap-2">
                                ...
                                <input type="file" accept="image/*" capture="environment" className="hidden" onChange={handleNativeScan} />
                            </label>
                ...
                <Link href={`/${lang}`} className="absolute top-4 left-4 bg-black/50 p-2 rounded-full z-20">✕</Link>
            </div>
                    ...
                    {items.map((item) => (
                        <div key={item.ean} className="bg-gray-900 rounded-lg p-3 flex gap-3 border border-gray-800">
                            <div className="flex-shrink-0 pt-1">
                                ...
                                {(item.status === 'found' || item.status === 'linked') && <span>✅</span>}
                            ...
                            <div className="flex-1 min-w-0">
                                ...
                                {(item.status === 'found' || item.status === 'contributed' || item.status === 'linked') && item.product && (
                                    <div>
                                ...
                                {item.status === 'unknown' && (
                                    <div>
                                        ...
                                        <div className="flex gap-2 mt-2">
                                            <button
                                                onClick={() => setSelectedItemForLink(item)}
                                                className="flex-1 text-xs bg-blue-900/40 border border-blue-700/50 px-2 py-2 rounded text-blue-200 hover:bg-blue-800/50"
                                            >
                                                🔍 Lier existant
                                            </button>
                                            <button
                                                onClick={() => setSelectedItemForContribute(item)}
                                                className="flex-1 text-xs bg-gray-800 border-gray-600 border px-2 py-2 rounded text-white hover:bg-gray-700"
                                            >
                                                + Créer
                                            </button>
                                        </div>
                            ...
                            {(item.status === 'found' || item.status === 'contributed' || item.status === 'linked') && item.product && item.product.image_url && (
            ...
            {/* MODALS */}
            {selectedItemForContribute && (
                <ContributeModal item={selectedItemForContribute} onClose={() => setSelectedItemForContribute(null)} onSubmit={handleContribute} />
            )}

            {selectedItemForLink && (
                <LinkProductModal item={selectedItemForLink} onClose={() => setSelectedItemForLink(null)} onSelect={handleLinkProduct} />
            )}
        </div>
    );
}
        ...
        <div className="fixed inset-0 z-50 bg-black/90 flex items-center justify-center p-4">
            <div className="bg-gray-900 rounded-xl w-full max-w-sm border border-orange-500/30 flex flex-col max-h-[90vh]">
                <div className="p-4 border-b border-gray-800 bg-gray-900 sticky top-0 rounded-t-xl z-20 flex justify-between">
                    <h3 className="font-bold text-white">Ajouter Nouveau</h3>
                    <button onClick={onClose} className="p-2">✕</button>
                </div>
                <div className="p-4 space-y-4 overflow-y-auto">
                    <p className="text-orange-300 text-center font-mono">{item.ean}</p>
                    <input className="w-full bg-black border border-gray-700 rounded p-3 text-white" placeholder="Nom du jeu" value={name} onChange={e => setName(e.target.value)} />
                    <input className="w-full bg-black border border-gray-700 rounded p-3 text-white" placeholder="Console (ex: PS2)" value={consoleName} onChange={e => setConsoleName(e.target.value)} />
...
function LinkProductModal({ item, onClose, onSelect }: { item: ScannedItem, onClose: () => void, onSelect: (p: any) => void }) {
    const [query, setQuery] = useState("");
    const [results, setResults] = useState<any[]>([]);
    const [loading, setLoading] = useState(false);

    // Auto-search if we had a partial name? No, just wait for user.
        ...
        <div className="fixed inset-0 z-50 bg-black/95 flex flex-col">
            <div className="p-4 bg-gray-900 flex items-center gap-3 border-b border-gray-800">
                ...
                <div className="flex-1">
                    <h3 className="text-sm font-bold text-gray-400">Lier le code {item.ean} à...</h3>
                    <input
                        autoFocus
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-2">
                {loading && <div className="text-center p-4 text-gray-500">Recherche...</div>}

                {!loading && results.length === 0 && query.length > 2 && (
                    <div className="text-center p-8 text-gray-600">Aucun résultat. Essayez un autre nom.</div>
                )}
                ...
                {results.map(p => (
                    <button
                        key={p.id}
                        onClick={() => onSelect(p)}
                        className="w-full bg-gray-800/50 p-3 rounded flex items-center gap-3 text-left hover:bg-gray-800"
                    >
                        {p.image_url ? (
                            <img src={p.image_url} className="w-10 h-14 object-cover rounded bg-black" />
                        ) : (
                            <div className="w-10 h-14 bg-gray-700 rounded flex items-center justify-center text-xs text-gray-500">?</div>
                        )}
                        <div>
                            <p className="font-bold text-white text-sm">{p.product_name}</p>
                            <p className="text-gray-400 text-xs">{p.console_name}</p>
                        </div>
                    </button>
                ))}
            </div>
