...
export async function generateMetadata({ params }: { params: Promise<{ system_slug: string }> }): Promise<Metadata> {
    ...
    return {
        title: `${shortSystemName} Price Guide & Value | RetroCharting`,
        description: `Current prices and value for ${systemName} games. Track your collection and see historic prices.`,
    };
}
                    ...
                    <div className="flex items-center justify-between mb-6 bg-[#1f2533] p-4 border-l-4 border-[#ff6600]">
                        <div>
                            <h1 className="text-2xl font-bold uppercase tracking-wider text-white">
                                {shortSystemName} <span className="text-[#ff6600]">Prices</span>
                            </h1>
                            <p className="text-xs text-gray-400 mt-1 font-mono">
                                Live tracking of {products.length} items. Updated daily.
                            </p>
                        </div>
                    </div>
